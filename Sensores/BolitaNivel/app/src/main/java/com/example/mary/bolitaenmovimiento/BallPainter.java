...
public class BallPainter extends SurfaceView {
    ...
    public BallPainter(Context context) {
        ...
        bolaLerp = BitmapFactory.decodeResource(getResources(), R.drawable.sakura);
    }
    ...
    protected void onDraw(Canvas lienzo) {
        ...
        xLerp=lerp(xLerp,x);
        yLerp=lerp(yLerp,y);
        System.out.println(yLerp);
        lienzo.drawBitmap(bolaLerp,xLerp,yLerp,null);
    }

    private float lerp(float currentX,float futureX){
        float distance=futureX-currentX;
        float near=.1f;
        if(Math.abs(distance)>near){
            int factor=20;
            return currentX+=distance/factor;
        }else
            return futureX;
    }
